# Object tracker based on Simple Object Tracking with OpenCv by pyimagesearch 
...
class ObjectTracker():
    ...
    # Initializes tracker
    def __init__(self, tolerance = float('Inf')):
    ...
    # Returns correspondence between centroid and object identifier
    def match(self, centroid):

        # Loop over the tracked objects
        for (id, c) in self.centroids.items():
            if centroid[0] == c[0] and centroid[1] == c[1]:
                return id

    # Plot detections with tracking
    def plot_detections(self, frame, coord_objs, idxs, det, names, colors):

        k = 0

        for *xyxy, conf, cls in det:

            # Plot only detections that have not been suppressed
            if k not in idxs:

                # Centroid of current object
                centroid = self.compute_centroids([coord_objs[k]])

                # Match detection
                id = self.match(centroid[0])

                if id != None:

                    # Add bbox to image
                    label = '%s %s (%.2f)' % (names[int(cls)], id, conf)
    ...
    def bbox_suppression(self, log, window = 10, thresh = 0.1):
        ...
        for (id, areas) in log.areas.items():
            ...
            print("Pearson's coefficient of variation:", variation(np.array(areas[-window:])))
            if variation(np.array(areas[-window:])) < thresh and self.changed[id] is False:
                idxs.append(id)
            else:
    ...
    def tracking(self, coord_objs):
        ...
        # Objects are already being tracked
            ...
            D = dist.cdist(np.array(object_centroids), centroids, metric = 'euclidean')
